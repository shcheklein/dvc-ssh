...
from dvc_ssh.tests.cloud import TEST_SSH_PASSWORD, TEST_SSH_USER
...
def test_password_auth_uses_configured_password_with_kbdint_only_server(
    kbdint_only_ssh_server,
    monkeypatch,
    tmp_path,
):
    monkeypatch.setenv("HOME", str(tmp_path))
    ...
    fs = SSHFileSystem(
        host=kbdint_only_ssh_server["host"],
        port=kbdint_only_ssh_server["port"],
        user=TEST_SSH_USER,
        password=TEST_SSH_PASSWORD,
    )
